...
class دن_مشا(ذریعہ):
    ...
    def __init__(خود, مسل, س_تاریخ, س_اعداد, تبادلوں):
        ...
        خود.ممکنہ_تاریخیں = (خود.دن[0], ft.timedelta(days=int(خود.دن[1][-1])) + خود.دن[0])

    def _اعداد_پیدا_کرنا(خود, سے, تک, **kwargs):
        شتونیں = [ب for ب in خود.دن_ستون if ب in خود.نام_شتونیں]
        شتونیں_اعداد = [خود.نام_شتونیں[ب] for ب in شتونیں]
        تاریخیں = پاندس.date_range(سے, تک)
        اعداد = پاندس.DataFrame(index=تاریخیں, data=خود.اعداد.اعداد_پانا(شتونیں=شتونیں_اعداد).T, columns=شتونیں)
...
class سال_مشا(ذریعہ):
    ...
    def __init__(خود, مسل, س_اعداد, س_سال, تبادلوں):
        ...
        خود.سال = خود.اعداد.اعداد_پانا(شتونیں=س_سال)[0].astype(int)

        خود.ممکنہ_تاریخیں = (ft.datetime(year=int(min(خود.سال)), month=1, day=1).date(),
                             ft.datetime(year=int(max(خود.سال)), month=1, day=1).date())

    def _اعداد_پیدا_کرنا(خود, سے, تک, **kwargs):

        ف_ستون = [س for س in خود.دن_ستون if س in خود.نام_شتونیں]
        شتونیں_اعداد = [خود.نام_شتونیں[ب] for ب in ف_ستون]

        اعداد_سال = خود.اعداد.اعداد_پانا(شتونیں=شتونیں_اعداد)
        ...
        for ش, س in enumerate(ف_ستون):
            if خود.تریقہ_جمع[س] == 'رقم':
                اعداد_سال[ش, :] = اعداد_سال[ش] / سال_میں_دن_شمار

        for ش, سال in enumerate(خود.سال.tolist()):
            for i, م in enumerate(ف_ستون):
                اعداد_دن.loc[اعداد_دن.index.year == سال, م] = اعداد_سال[i, ش]

        for م, ش in خود.تبادلوں.items():
            اعداد_دن[م] = اعداد_دن[م] * ش

        return اعداد_دن[سے:تک]
