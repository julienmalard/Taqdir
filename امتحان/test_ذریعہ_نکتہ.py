...
class امتحان_ذریعہ_نکتہ(unittest.TestCase):
    ...
    def setUpClass(قسم):
        ...
        قسم.اعداد['درجہ_حرارت_اوسط'] = np.nan
    ...
    def test_کوائف_پانا(خود):
        for نام, ذریعہ in خود.ذرائع.items():
            with خود.subTest(نام):
                pdt.assert_frame_equal(ذریعہ.کوائف_پانا('۲۰۱۳۰۱۰۱', '۲۰۱۳۰۱۰۹', *خود.جگہ), خود.اعداد, check_like=True)

    def test_بلندی_سحی_نہیں(خود):
        for نام, ذریعہ in خود.ذرائع.items():
            with خود.subTest(نام):
                خود.assertIsNone(ذریعہ.کوائف_پانا('۲۰۱۳۰۱۰۱', '۲۰۱۳۰۱۰۹', *خود.جگہ[:-1], بلندی=-10))

    def test_خاکے_سہی_نہیں(خود):
        for نام, ذریعہ in خود.ذرائع_خاکے.items():
            with خود.subTest(نام):
                خود.assertIsNone(ذریعہ.کوائف_پانا('۲۰۱۳۰۱۰۱', '۲۰۱۳۰۱۰۹', *خود.جگہ, خاکے='۸۔۰'))

    def testسہی_خاکے_(خود):
        for نام, ذریعہ in خود.ذرائع_خاکے.items():
            with خود.subTest(نام):
                pdt.assert_frame_equal(
                    ذریعہ.کوائف_پانا('۲۰۱۳۰۱۰۱', '۲۰۱۳۰۱۰۹', *خود.جگہ, خاکے='۲۔۶'), خود.اعداد, check_like=True
                )
...
class امتحان_ماہانہ(unittest.TestCase):
    ...
    def test_کوائف_پانا(خود):
        for نام, ذریعہ in خود.ذرائے.items():
            with خود.subTest(نام):
                pdt.assert_frame_equal(ذریعہ.کوائف_پانا('۱۹۳۱۰۱۰۱', '۱۹۳۱۰۲۲۸', *خود.جگہ), خود.اعداد, check_like=True)
...
class امتحان_سالانہ(unittest.TestCase):
    ...
    def test_کوائف_پانا(خود):
        for نام, ذریعہ in خود.ذرائے.items():
            with خود.subTest(نام):
                pdt.assert_frame_equal(ذریعہ.کوائف_پانا('۲۰۱۲۰۱۰۱', '۲۰۱۳۱۲۳۱', *خود.جگہ), خود.اعداد, check_like=True)
